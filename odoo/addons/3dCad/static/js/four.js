...
FOUR.Viewcube = (function () {
    ...
    function Viewcube(config) {
        ...
        self.domElement = config.domElement;
        self.width = self.domElement.clientWidth;
        self.height = self.domElement.clientHeight;
        if (config.width!='undefined'){self.width=config.width;}
        if (config.height!='undefined'){config.height=config.height;}
        self.enabled = false;
        self.fov = 60; // 50
        self.frontFace = null;
        self.labelSize = 128;
    ...
    Viewcube.prototype.setupCamera = function () {
        var self = this;
        self.camera = new THREE.PerspectiveCamera(self.fov, self.width / self.height, 0.1, 1000);
    ...
    Viewcube.prototype.setupGeometry = function () {
        ...
        if (self.display.controlAxis) {
            var controlAxis = new THREE.AxesHelper(100);
            self.cube.add(controlAxis);
        }

        if (self.display.sceneAxis) {
            var sceneAxis = new THREE.AxesHelper(150);
            self.scene.add(sceneAxis);
        }

        if (self.display.cameraAxis) {
            var cameraAxis = new THREE.AxesHelper(100);
    ...
    Viewcube.prototype.setupMaterials = function () {
        ...
        var label1 = new THREE.MeshPhongMaterial({
            color: 0xAAAAAA,
            map: new THREE.TextureLoader().load('/3dCad/static/js/libFour/img/' + self.labelSize + '/top.png'),
            opacity: self.LABELS_HOVER_OFF,
            transparent: true
        });
        var label2 = new THREE.MeshPhongMaterial({
            color: 0xAAAAAA,
            map: new THREE.TextureLoader().load('/3dCad/static/js/libFour/img/' + self.labelSize + '/front.png'),
            opacity: self.LABELS_HOVER_OFF,
            transparent: true
        });
        var label3 = new THREE.MeshPhongMaterial({
            color: 0xAAAAAA,
            map: new THREE.TextureLoader().load('/3dCad/static/js/libFour/img/' + self.labelSize + '/right.png'),
            opacity: self.LABELS_HOVER_OFF,
            transparent: true
        });
        var label4 = new THREE.MeshPhongMaterial({
            color: 0xAAAAAA,
            map: new THREE.TextureLoader().load('/3dCad/static/js/libFour/img/' + self.labelSize + '/left.png'),
            opacity: self.LABELS_HOVER_OFF,
            transparent: true
        });
        var label5 = new THREE.MeshPhongMaterial({
            color: 0xAAAAAA,
            map: new THREE.TextureLoader().load('/3dCad/static/js/libFour/img/' + self.labelSize + '/back.png'),
            opacity: self.LABELS_HOVER_OFF,
            transparent: true
        });
        var label6 = new THREE.MeshPhongMaterial({
            color: 0xAAAAAA,
            map: new THREE.TextureLoader().load('/3dCad/static/js/libFour/img/' + self.labelSize + '/bottom.png'),
            opacity: self.LABELS_HOVER_OFF,
            transparent: true
        });
        self.materials.labels = [label3, label4, label5, label2, label1, label6];
    };
