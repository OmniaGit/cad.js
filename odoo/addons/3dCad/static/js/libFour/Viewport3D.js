...
FOUR.Viewport3D = (function () {
    ...
    function Viewport3D(config) {
        ...
        self.renderer.setSize(self.domElement.width, self.domElement.height);
        ...
        if (config.inDomElement==true){
            self.domElement.addEventListener(FOUR.EVENT.RESIZE, self.onWindowResize.bind(self), false);
        }
        else{
    ...
    Viewport3D.prototype.onWindowResize = function () {
        var ctrl, self = this;
        var height = self.domElement.height;
        var width = self.domElement.width;
